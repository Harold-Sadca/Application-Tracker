...
import { createInterview, getInterview, updateInterview } from "../models/methods/interviewMethods";
...
export const createInterviewController = async(req:Request, res:Response) => {
  try {
    ...
    if (id && date && time) {
      ...
      const newInterview = await createInterview(id, interview)
...
export const updateInterviewController = async (req:Request, res:Response) => {
  try {
    const {id} = req.params
